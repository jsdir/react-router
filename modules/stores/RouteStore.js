...
var RouteStore = {
  ...
  registerRoute: function (route, _parentRoute) {
    ...
    props.displayName = props.name || props.path;
    ...
    if (props.path || props.name) {
      ...
      props.path = _parentRoute ? _parentRoute.props.path : '/';

      // Default routes have no name, path, or children.
      isDefault = !props.children;
    }
    ...
    if (_parentRoute && isDefault) {
      invariant(
        ...
        'You may not have more than one <DefaultRoute> under route %s',
        _parentRoute.displayName
      );
